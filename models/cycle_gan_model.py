...
class CycleGANModel(BaseModel):
        ...
        if self.isTrain:
            ...
            if self.opt.patch_N > 0:
                ...
                self.optimizer_D_P = torch.optim.AdamW(self.netD_P.parameters(),
                                                lr=opt.lr_d, betas=(opt.beta1, 0.999), weight_decay=opt.wd)
    ...
    def backward_D_A(self):
        ...
        if self.opt.patch_N > 0:
            loss_D_patch = 0
            for i in range(self.opt.patch_N):
                loss_D_patch += self.backward_D_basic(self.netD_P, self.real_B_patch[i], self.fake_B_patch[i], self.opt.loss)
    ...
    def backward_G(self):
        ...
        # combined loss
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_style + self.loss_G_patch
