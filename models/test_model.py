...
class TestModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG,
                                      opt.norm_g, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, True, opt.spectral_norm, opt.attention, opt.gray)
        self.netD = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                                    opt.n_layers_D, opt.norm_d, opt.no_lsgan, opt.init_type, opt.init_gain, self.gpu_ids, opt.spectral_norm, opt.attention, opt.gray, opt.ogray)
    ...
    def set_input(self, input):
        ...
        elif self.opt.gray or self.opt.ogray or self.opt.igray:
    ...
    def forward(self):
        if self.opt.exposure:
            if self.opt.amp:
                ...
                self.fake_B = self.netG(self.real_A_over_exposure, self.real_A_normal_exposure, self.real_A_under_exposure)
        elif self.opt.gray or self.opt.igray:
            self.fake_B = self.netG(self.real_A, gray_img=self.real_A_gray)
