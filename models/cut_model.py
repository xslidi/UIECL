...
class CUTModel(BaseModel):
    ...
    def backward_D(self):
        ...
        self.loss_D, self.loss_D_real, self.loss_D_fake = self.backward_D_basic(self.netD, self.real_B, fake_B, self.opt.loss)
                            
        if self.opt.patch_N > 0:
            loss_D_patch = 0
            for i in range(self.opt.patch_N):
                loss_D_patch += self.backward_D_basic(self.netD_P, self.real_B_patch[i], self.fake_B_patch[i], self.opt.loss)[0]
    ...
    def backward_G(self):
        ...
        # combined loss
        self.loss_G = self.loss_G + self.loss_idt_B + loss_NCE_both + self.loss_G_patch
